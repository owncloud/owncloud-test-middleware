...
exports.delete = async function (userId, file) {
  ...
  if (deleteTimestamps[userId] && deleteTimestamps[userId][filename]) {
    ...
    if (timeSinceLastDelete < 1001) {
      await testHelper.customDelay(1001 - timeSinceLastDelete);
    }
  }
...
exports.getTrashBinElements = function (user, depth = 2) {
  return new Promise((resolve, reject) => {
    exports
      ...
      .then((str) => {
        const trashData = convert.xml2js(str, { compact: true })[
          "d:multistatus"
        ]["d:response"];
        const trashItems = [];
        trashData.map((trash) => {
          if (trash["d:propstat"]["d:prop"] === undefined) {
            ...
            trashItems.push({
              ...
              originalFilename:
                trash["d:propstat"]["d:prop"]["oc:trashbin-original-filename"]
                  ._text,
              originalLocation:
                trash["d:propstat"]["d:prop"]["oc:trashbin-original-location"]
                  ._text,
              deleteTimestamp:
                trash["d:propstat"]["d:prop"]["oc:trashbin-delete-timestamp"]
                  ._text,
              lastModified:
                trash["d:propstat"]["d:prop"]["d:getlastmodified"]._text,
            });
          }
        });
        resolve(trashItems);
...
exports.getSkeletonFile = function (filename) {
  return occHelper
    ...
    .then((dir) => {
      ...
      const apiURL = `apps/testing/api/v1/file?file=${encodeURIComponent(
        element
      )}&absolute=true`;
...
exports.getFavouritedResources = function (user) {
  ...
  return httpHelper
    ...
    .then((res) => res.text())
    .then((res) => {
      const favData = convert.xml2js(res, { compact: true })["d:multistatus"][
        "d:response"
      ];
      const favItems = [];
      favData.map((favourite) => {
        favItems.push({
          ...
          isFolder: !(
            Object.keys(favourite["d:propstat"]["d:prop"]["d:resourcetype"])
              .length === 0
          ),
        });
      });
