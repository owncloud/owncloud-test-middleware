...
exports.download = async function (userId, file) {
  ...
  let retryCounter = 0
  const isRetryAllowed = () => retryCounter++ < 10 && [423, 425].includes(res.status)
  while (isRetryAllowed()) {
    if (res.status === 423) {
      console.info(`Resource is locked. Retrying (${retryCounter})...`)
    } else if (res.status === 425) {
      console.info(`Resource is in postprocessing. Retrying (${retryCounter})...`)
    }
    ...
    .then((res) => {
      ...
      favData.forEach((favourite) => {
        favItems.push({
          ...
          isFolder: !(
            Object.keys(favourite['d:propstat']['d:prop']['d:resourcetype']).length === 0
          ),
        })
      })
      return favItems
