...
const assertRemoteFileSameAsSkeletonFile = async function (
  userId,
  remoteFile,
  skeletonFile
) {
...
Given(
  "the administrator has cleared the versions for user {string}",
  function (userId) {
    if (client.globals.ocis) {
      // TODO: decide if we fail on OCIS when a scenario even tries to use this given step
      return;
...
Given(
  "default expiration date for users is set to {int} day/days",
  function (days) {
    if (client.globals.ocis) {
      // TODO: decide if we fail on OCIS when a scenario even tries to use this given step
      return;
