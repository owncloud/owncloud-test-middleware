...
Then(
  ...
  async function (permissions, user, resource) {
    await client.page.FilesPageElement.appSideBar().closeSidebar(100).openSharingDialog(resource)
...
When('the user shares with the selected collaborators', function () {
  return client.page.FilesPageElement.sharingDialog().confirmShare().waitForOutstandingAjaxCalls()
})
