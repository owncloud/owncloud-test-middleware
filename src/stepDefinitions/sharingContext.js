...
const userSharesFileOrFolderWithUserOrGroup = async function (
  ...
  if (quickAction) {
    await client.page.FilesPageElement.filesList().useQuickAction(
      file,
      "collaborators"
    );
  } else {
    await api.appSideBar().closeSidebar(100).openSharingDialog(file);
  }
...
const shareFileFolder = async function (
  ...
  const permissions = sharingHelper.humanReadablePermissionsToBitmask(
    permissionString
  );
  ...
  await httpHelper
    ...
    .then(function (json) {
      httpHelper.checkOCSStatus(
        json,
        "Could not create share. Message: " + json.ocs.meta.message
      );
    });
...
Given(
  ...
  function (sharer, dataTable) {
    ...
    if (settings.shareTypeString) {
      targetShareType = sharingHelper.humanReadableShareTypeToNumber(
        settings.shareTypeString
      );
    }
...
When(
  ...
  async function (collaborator, resource, dataTable) {
    const settings = dataTable.rowsHash();
    const api = client.page.FilesPageElement;
    await api.appSideBar().closeSidebar(100).openSharingDialog(resource);
    return api
      .sharingDialog()
      .changeCollaboratorSettings(collaborator, settings);
  }
);
  ...
  return client.page.FilesPageElement.SharingDialog.collaboratorsDialog()
    ...
    .then((shares) => {
      const share = shares.find((share) => {
        return (
          name === share.displayName && type === share.shareType.toLowerCase()
        );
      });
      ...
      if (!share) {
        assert.fail(
          `"${name}" with type "${type}" was expected to be in share list but was not present. Found collaborators: ` +
            shares.map(
              (share) => `name=${share.displayName} type=${share.shareType}`
            )
        );
      }
...
const assertCollaboratorslistDoesNotContain = function (type, name) {
  ...
  return collaboratorsDialog
    .getCollaboratorsList(
      {
        displayName:
          collaboratorsDialog.elements.collaboratorInformationSubName,
      },
      name,
...
const getUsersMatchingPattern = function (pattern) {
  ...
  return usersID
    .filter(
      (id) =>
        users[id].displayname.toLowerCase().includes(pattern) ||
        id.includes(pattern)
    )
...
const getGroupsMatchingPattern = function (pattern) {
  ...
  const groupMatchingPattern = groups.filter((group) =>
    group.toLowerCase().includes(pattern)
  );
...
const assertUsersGroupsWithPatternInAutocompleteListExcluding = async function (
  ...
  const currentUserDisplayName = userSettings.getDisplayNameForUser(
  ...
  const usersMatchingPattern = getUsersMatchingPattern(pattern).filter(
    (displayName) => {
      return (
        ...
        displayName !== alreadySharedUserOrGroup
      );
    }
  );
  ...
  return sharingDialog
    ...
    .assertAlreadyExistingCollaboratorIsNotInAutocompleteList(
      alreadySharedUserOrGroup,
      userOrGroup
    );
};
 ...
 * @param {string} value
 */
const checkCollaboratorsExpirationDate = async function (
  collaborator,
  resource,
  value
) {
...
const checkReceivedSharesExpirationDate = function (user, target, days) {
  ...
  return httpHelper
    ...
    .then(function (result) {
      httpHelper.checkOCSStatus(
        result,
        "Could not get shares. Message: " + result.ocs.meta.message
      );
      ...
      expectedExpirationDate = new Date(
        expectedExpirationDate.setHours(0, 0, 0, 0)
      );
      let found = false;
      const foundDates = [];
      ...
      for (const share of shares) {
        ...
        if (share.expiration) {
          foundDates.push(
            `${share.file_target.substr(1)}: ${new Date(share.expiration)}`
          );
        }
      }
      ...
      const message = found
        ...
        : `Expected ${target}: ${expectedExpirationDate}. Found ${
            foundDates || "none"
          }`;
...
Given(
  "the administrator has excluded group {string} from sharing",
  async function (group) {
    ...
    if (!initialExcludedGroup.includes(group)) {
      ...
      const resultGroupList = initialExcludedGroup.map(
        (res) => '"' + res + '"'
      );
...
const assertSharePermissions = async function (
  ...
  for (let i = 0; i < COLLABORATOR_PERMISSION_ARRAY.length; i++) {
    ...
    if (permissions !== undefined) {
      ...
      if (expectedPermissionArray.includes(permissionName)) {
        ...
        assert.ok(
          !currentSharePermissions[permissionName],
          `Permission ${permissionName} is set`
        );
      }
    } else {
...
When(
  "the user removes {string} as a collaborator from the share",
  function (user) {
    return client.page.FilesPageElement.sharingDialog().removePendingCollaboratorForShare(
      user
    );
  }
);
        ...
        itemsList.forEach((item) => {
          ...
          userSettings.getCreatedGroups().forEach(function (groupId) {
            if (
              ...
              groupId.toLowerCase().includes(pattern)
            ) {
              found = true;
            }
          });
...
When("the user selects role {string}", function (role) {
  return client.page.FilesPageElement.sharingDialog().selectRoleForNewCollaborator(
    role
  );
});
...
Then(
  ...
  async function (item, user) {
    ...
    await client.page.FilesPageElement.appSideBar().selectAccordionItem(
      "people"
    );
...
Then(
  ...
  function (user, role, resharedThrough) {
    return assertCollaboratorslistContains("user", user, {
      role,
      resharedThrough,
    });
  }
...
Then(
  "user {string} should not be listed in the collaborators list on the webUI",
  function (user) {
...
Then(
  ...
  function (user, expectedDetailsTable) {
    return sharingHelper.assertUserHasShareWithDetails(
      user,
...
Then(
  ...
  async function (resource) {
    ...
    const noSharePermissionsMsgFormat =
      "You don't have permission to share this %s.";
    const noSharePermissionsFileMsg = util.format(
      noSharePermissionsMsgFormat,
      "file"
    );
    const noSharePermissionsFolderMsg = util.format(
      noSharePermissionsMsgFormat,
      "folder"
    );
    return assert.ok(
...
Then(
  ...
  async function (resource) {
    ...
    const count = (
      await api.SharingDialog.collaboratorsDialog().getCollaboratorsList({})
    ).length;
    assert.strictEqual(
      count,
      0,
...
Then(
  "the expiration date field should be marked as required on the WebUI",
  async function () {
...
Then(
  "the current collaborators list should have order {string}",
...
When(
  ...
  function (filename, user) {
    return client.page
      .sharedWithMePage()
      .declineAcceptFile("Decline", filename, user);
  }
);
...
When(
  ...
  function (filename, user) {
    return client.page
      .sharedWithMePage()
      .declineAcceptFile("Accept", filename, user);
  }
);
...
Then(
  "the autocomplete list should not be displayed on the webUI",
  async function () {
    const isVisible = await client.page.FilesPageElement.sharingDialog().isAutocompleteListVisible();
    return assert.ok(
      !isVisible,
      'Expected: autocomplete list "not visible" but found "visible"'
    );
  }
);
...
Given(
  ...
  function (user, server) {
    if (server === backendHelper.BACKENDS.remote) {
      return backendHelper.runOnRemoteBackend(() =>
        sharingHelper.acceptLastPendingShare(user)
      );
    } else {
...
Then(
  "the file {string} shared by {string} should not be in {string} state",
  async function (filename, sharer, expectedStatus) {
    const currentStatus = await client.page
      .sharedWithMePage()
      .getShareStatusOfResource(filename, sharer);
...
Then(
  ...
  async function (element, sharer) {
    const username = await client.page
      .sharedWithMePage()
      .getSharedByUser(element);
    return assert.strictEqual(
      username,
      sharer,
...
Then("user {string} should not have created any shares", async function (user) {
  ...
  assert.strictEqual(
    shares.length,
    0,
    "There should not be any share, but there are"
  );
});

Then(
  "the following resources should have the following collaborators",
  async function (dataTable) {
    for (const { fileName, expectedCollaborators } of dataTable.hashes()) {
      ...
      const expectedCollaboratorsArray = expectedCollaborators
        .split(",")
        .map((s) => s.trim());
      assert.ok(
        _.intersection(collaboratorsArray, expectedCollaboratorsArray)
          .length === expectedCollaboratorsArray.length,
...
When(
  "the user opens the sharing sidebar for file/folder {string}",
  function (file) {
...
Then(
  "it should not be possible to save the pending share on the webUI",
  async function () {
...
Given(
  ...
  function (folder) {
    if (client.globals.ocis) {
      ...
      throw Error(
        `Cannot set ${folder} as default share received folder in OCIS`
      );
    }
