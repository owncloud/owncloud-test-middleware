...
const fileShouldNotExist = function (userId, element) {
  return fileExists(userId, element).then(function (res) {
    assert.ok(
      ...
      "file/folder should not exist, but does"
    );
  });
};

Then(
  "as {string} file/folder {string} should not exist",
...
Then(
  "as {string} file/folder {string} should exist",
  function (userId, element) {
...
Then(
  "as {string} file/folder {string} should exist on remote server",
  function (userId, element) {
...
Then(
  ...
  async function (user, files) {
    ...
    const filesNotFoundInUploads = _.difference(
      files,
      uploadedFilesWithBaseName
    );
    assert.strictEqual(
...
Given(
  "user {string} has favorited element {string}",
  function (userId, element) {
    const body =
      '<?xml version="1.0"?>\n' +
