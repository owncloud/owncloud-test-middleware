...
Given(
  "user {string} has been created with default attributes on remote server",
  function (userId) {
...
Given(
  "the quota of user {string} has been set to {string}",
  function (userId, quota) {
    const body = new URLSearchParams();
    ...
    return Promise.all(
      dataTable.rows().map((userId) => {
        return deleteUser(userId.toString()).then(() =>
          createDefaultUser(userId.toString())
        );
      })
    );
  }
...
Given(
  "these users have been created but not initialized:",
  function (dataTable) {
...
Given(
  "these users have been created with default attributes:",
  function (dataTable) {
    return Promise.all(
...
Given(
  "user {string} has been added to group {string}",
...
After(async function () {
  ...
  const createdRemoteUsers = Object.keys(
    userSettings.getCreatedUsers("REMOTE")
  );
  ...
  if (client.globals.ocis) {
    const deleteSharePromises = createdUsers.map((user) => {
      return sharingHelper.getAllSharesSharedByUser(user).then((shares) => {
        if (shares.length) {
          return Promise.all(
            shares.map((share) => sharingHelper.deleteShare(share.id, user))
          );
        }
      });
    });
  ...
  if (client.globals.ldap) {
    ...
    const deleteUserDirectories = createdUsers.map((user) =>
      fs.remove(dataDir(user))
    );
    ...
    await Promise.all([
      ...createdUsers.map(deleteUser),
      ...createdGroups.map(deleteGroup),
    ]);
  }
