...
describe('Creating data tables', () => {
  it.each(tableData)('Only valid tables can be created', (tableData, expected) => {
    try {
      const table = new Table(tableData.data)
      if (!expected) {
        throw new Error('table should not be created')
      }
      expect(table.data).toBe(tableData.data)
    } catch (err) {
      if (expected) {
        throw new Error('Table should have been created but failed')
      }
...
describe('Table.rows()', () => {
  it.each(tableData)('Table.rows() returns correct data', (tableData, expected) => {
    if (!expected) {
      return
    }
    const table = new Table(tableData.data)
...
describe('Table.hashes()', () => {
  it.each(tableData)('Table.hashes() returns correct data', (tableData, expected) => {
    if (!expected) {
      return
    }
    const table = new Table(tableData.data)
...
describe('Table.rowsHash()', () => {
  it.each(tableData)('Table.rowsHash() returns correct data', (tableData, expected) => {
    if (!expected) {
      return
    }
    try {
      ...
      if (!tableData.rowsHash) {
        throw new Error('rowsHash should not be present for table with more than 2 columns')
      }
      ...
      expect(err.message).toBe('Cannot perform rowsHash on table with more than 2 columns')
    }
  })
})
