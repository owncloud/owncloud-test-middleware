...
const getMatchData = [
  ...
  },
  {
    addStep: {
      token: "WHEN",
      step:
        "user {string} shares folder {string} with user {string} with {string} permissions",
...
describe("test testContext", () => {
  it.each(testContextData)(
    "test add step for different action types",
    (Data) => {
      ...
      if (Data.action === "function") {
        ...
        try {
          ...
          fail(
            "Using action of not a function type should fail but it passed."
          );
        } catch (err) {
  ...
  it("test add step", () => {
    ...
    for (let i = 0; i < expectedSteps.length; i++) {
      ...
      expect(testContext.steps[i].pattern).toStrictEqual(
        expectedSteps[i].pattern
      );
      expect(testContext.steps[i].action).toStrictEqual(
        expectedSteps[i].action
      );
    }
  });
  ...
  it("test add step to check already existing step", () => {
    ...
    try {
      ...
      expect(err.message).toBe("step already registered");
    }
  });
  ...
  it("test given when and then functions", () => {
    ...
    testContext.when(
      "admin creates a user {string} with default attributes",
      actionFunction
    );
